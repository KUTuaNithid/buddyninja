...
## 1. Compare MQTT and HTTP for IoT data transmission. Which would you choose for batterypowered GPS trackers? Why?
  I would choose the MQTT. The device has low resource, so we need the lightweight communication model.
  We need to minimize the power consumption and the bandwidth.
  Reasons
  ...
  4. Community
  - They have a lot of approch using MQTT for the IoT device, so why not ?
## 2. How would you reconnect an ESP32 automatically when Wi-Fi connection drops?
  ...
  In the disconnected state, we may have a process to collect the crucial information.
  We can save this information in the limited memory and we can upload this data when the connection is back.
  ...
  After WIFI is reconnected, we should reconnect the communication(MQTT) as well.
  
## 3. Describe how you would implement over-the-air (OTA) firmware updates securely for ESP32 IoT devices.
The crucial things that we need to concern are 2 things.
1. Error handling
We need a mechanism that the firmware does not lost if the updating is interrupted by something.
For the worst case scenario, we could be able roll back to the previous one.
...
ESP32 can do the deep sleep. For example, we can turn of the WIFI, Bluetooth, cut off the power to the external device. This operation could take sometime. Either for terminate or initialize again.
...
We have the IMU in the system, so I would say Kalman filter is the useful method.
When the GPS data "jumps," the filter sees that this jump doesn't match the IMU prediction, so it rejects the GPS jump as noise.
...
The kalmain filter is strong for the outlier. If there is a jump, they can filter out.
Anyway, we can adjust the weight, how would they react to the outlier.